...
if(!s.reps && !s.seenIntro){
  ...
  setTimeout(function(){
    ...
    '<b>3 · Start with the warmup.</b> Six minutes, mechanical, no scoring. It is the difference between practising your voice and practising your tired voice.</div>'+
    ...
    '<button class="btn" id="introMic">Turn on the mic &amp; start</button>'+
    '<button class="btn gh" onclick="UI.closeModal()">Look around first</button></div>');
    UI.$('#introMic').onclick=function(){ UI.closeModal(); UI.needMic().then(function(ok){ if(ok) Drill.launch('warmup'); }); };
  }, 500);
}
